"""
Test mass stripping and orbit integration (evolve.py and orbit.py) in
potential wells defined in profiles.py. Runs a single satellite through
a static host potential and produces diagnostic plots.

Usage:
    python test_evolve.py [--save]

Without --save: shows interactive plot (requires display / Qt5Agg).
With --save: uses Agg backend and writes FIGURE/test_evolve.pdf.
"""
...
mpl.use('Agg' if '--save' in sys.argv else 'Qt5Agg')
...
cfg.Mres = 1e6 # [Msun] mass resolution for evolution loop
